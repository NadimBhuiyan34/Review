      ...
      <div class="space-y-4 rounded-2xl bg-white p-6 shadow-md">
        ...
        <div v-if="filteredReviews.length" class="grid gap-4 md:grid-cols-2">
          <div
            ...
            :key="index"
            class="rounded-xl border border-gray-100 p-4 transition hover:shadow-md"
          >
            <div class="mb-2 flex items-center justify-between">
              <span class="font-semibold text-gray-800">{{ review.name }}</span>
              <span
                :class="{
                  'text-green-600': review.sentiment === 'Good',
                  'text-yellow-500': review.sentiment === 'Neutral',
                  'text-red-600': review.sentiment === 'Bad',
                }"
...
const filteredReviews = computed(() => {
  ...
  return reviews.value.filter(r => r.sentiment === selectedFilter.value)
})
